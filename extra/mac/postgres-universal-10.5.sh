...
CFLAGS="-arch ppc -arch i386 $COMMON_CFLAGS" LDFLAGS="$COMMON_LDFLAGS" ./configure --prefix=$PREFIX --with-openssl --without-perl --without-python --without-tcl --without-bonjour --without-pam --with-krb5 --without-libxml || die "Failed to configure Postgres for Universal"
...
find . -name Makefile -print -exec perl -p -i.backup -e 's/\Q$(LD) $(LDREL) $(LDOUT)\E (\S+) (.+)/\$(LD) -arch ppc \$(LDREL) \$(LDOUT) $1.ppc $2; \$(LD) -arch i386 \$(LDREL) \$(LDOUT) $1.i386 $2; lipo -create -output $1 $1.ppc $1.i386/' {} \; || die "Failed to post-process the Postgres Makefiles for Universal build"
