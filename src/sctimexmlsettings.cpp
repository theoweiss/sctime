...
void SCTimeXMLSettings::writeShellSkript(AbteilungsListe* abtList)
{
  if (abtList->checkInState()) {
      QMessageBox::warning(NULL, QObject::tr("sctime: save"),
          QObject::tr("Shell script not written because it has already been checked in."));
      return;
  }
...
void SCTimeXMLSettings::writeSettings(bool global, AbteilungsListe* abtList)
{
  if ((abtList->checkInState())&&(!global)) {
      QMessageBox::warning(NULL, QObject::tr("sctime: writing XML file"),
          QObject::tr("file not written because it has already been checked in"));
      return;
  }
  #ifndef NO_XML
  QDomDocument doc("settings");
